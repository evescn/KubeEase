...
func (c *configmap) GetConfigMaps(client *kubernetes.Clientset, filterName, namespace string, size, page int) (configMapsResp *vo.ConfigMapsResp, err error) {
	...
	//实例化dataSelector对象，把 d 结构体中获取到的 ConfigMap 列表转化为 dataSelector 结构体，方便使用 dataSelector 结构体中 过滤，排序，分页功能
	...
	//将[]DataCell类型的 ConfigMap 列表转为 v1.ConfigMap 列表
	configMaps := c.fromCells(data.GenericDataList)
	return &vo.ConfigMapsResp{
		Items: configMaps,
		Total: total,
	}, nil
}
