...
// DataSelectQuery 定义过滤和分页的属性，过滤：Name，分页：Size 和 page
...
func (d *dataSelector) Less(i, j int) bool {
	iTmp := d.GenericDataList[i].GetCreation()
	jTmp := d.GenericDataList[i].GetCreation()
	return iTmp.Before(jTmp)
}
...
// Paginate 方法用于数组分页，根据Size和Page的传参，返回数据
func (d *dataSelector) Paginate() *dataSelector {
	size := d.dataSelectQuery.PaginateQuery.Size
...
// podCell 定义 podCell 类型，实现两个方法 GetCreation GetName，可进行类型转换
