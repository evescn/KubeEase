...
// GetList 获取列表
func (*event) GetList(name, cluster string, page, size int) (*dao.Events, error) {
	data, err := dao.Event.GetList(name, cluster, page, size)
	if err != nil {
		return nil, err
	}
	return data, nil
...
func onAdd(obj interface{}, cluster string) {
	...
	// 判断是否重复 name, kind, namespace, reason string, eventTime time.Time, cluster string
