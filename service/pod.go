...
// GetPods 获取pod列表，支持过滤，排序，分页，client 用于选择哪个集群
func (p *pod) GetPods(client *kubernetes.Clientset, filterName, namespace string, size, page int) (podsResp *vo.PodsResp, err error) {
	...
	//实例化 dataSelector 对象，把 p 结构体中获取到的 Pod 列表转化为 dataSelector 结构体，方便使用 dataSelector 结构体中 过滤，排序，分页功能
