...
// GetPvcs 获取 PVC 列表
func (c *pvc) GetPvcs(client *kubernetes.Clientset, filterName, namespace string, size, page int) (pvcsResp *vo.PvcsResp, err error) {
	...
	if err != nil {
		zap.L().Error(fmt.Sprintf("获取 PVC 列表失败, %v\n", err))
		return nil, errors.New(fmt.Sprintf("获取 PVC 列表失败, %v\n", err))
	}
	//实例化dataSelector对象，把 d 结构体中获取到的 Pvc 列表转化为 dataSelector 结构体，方便使用 dataSelector 结构体中 过滤，排序，分页功能
	...
	//将[]DataCell类型的 Pvc 列表转为 v1.Pvc 列表
